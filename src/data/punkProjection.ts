...
import type { CareerProjection, CareerStarStoryProjection, ConnectionLaneProjection, ConnectionPersonProjection, ConnectionPriority, EducationDeadlineProjection, IdentityQualityProjection, ProjectedSection, SystemsTaskProjection, VentureProjection, VesselMuscleGroupProjection } from './projectedTypes'
...
function familyConnectionProfileSummary(person: string, relationship: string, location: string, closeness: string, contact: string, lane: string) {
...
function buildFamilyConnectionPeople(): ConnectionPersonProjection[] {
  const immediateFamily = [
    {
      ...
      priority: 'active' as ConnectionPriority,
    ...
    },
    {
      ...
      priority: 'high' as ConnectionPriority,
      lane: 'Immediate family',
    },
    {
      person: 'Melvin',
  ...
  const extendedFamily = [
    ...
    priority: 'medium' as ConnectionPriority,
...
function parseVenturePortfolio(ventures: string, scorecard: string): VentureProjection[] {
  ...
  return rows.map((row, index) => {
    ...
    const priorityBand: VentureProjection['priorityBand'] = priorityRank === 1
      ? 'primary'
      : priorityRank <= 3
        ? 'secondary'
...
function parseScoreRows(scorecard: string) {
  ...
  const scores = new Map<string, { rawScore: string; adjustedScore: string }>()
  scorecard
    .split('\n')
...
function nextActionForVenture(name: string, blocker: string, priorityBand: VentureProjection['priorityBand']) {
...
function detailForVenture(name: string, stage: string, type: string) {
...
function parseStarStoryFields(block: string, occurrence = 0) {
...
function parseStarStories(markdown: string): CareerStarStoryProjection[] {
